...
import { DoctorsService, Doctor } from '../../sevices/doctors.service';
  ...
  protected saveDoctor() {
    ...
    if (editing) {
      this.doctorsService.update(editing.id, { name, specialty }).subscribe({
        ...
        error: () =>
          this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to update doctor' }),
      });
    } else {
      this.doctorsService.create({ name, specialty }).subscribe({
        ...
        error: () =>
          this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to create doctor' }),
      });
    }
  }
